...
class AmbienteRepositorio
{
    ...
    public function consultarNomeAmbientePorId($id) {
        $this->Conexao->abrirConexao();
        $codigoSQL = "SELECT * FROM AMBIENTE" .
            " WHERE ID = " . $id . ";";
        $numeroLinhas = mysqli_num_rows($this->Conexao->getConexao()->query($codigoSQL));
        if ($numeroLinhas > 0) {
            ...
            $AmbienteDestino->setId($linha['ID']);
            $Unidade = new Unidade();
            $Unidade->setId($linha['IDUNIDADE']);
            $AmbienteDestino->setUnidade($Unidade);
            $AmbienteDestino->setNome($linha['NOME']);
            $AmbienteDestino->setDescricao($linha['DESCRICAO']);
            $AmbienteDestino->setCodigoBarra($linha['CODIGOBARRA']);
            $AmbienteDestino->setDtRegistro($linha['DTREGISTRO']);
            $AmbienteDestino->setSituacao($linha['SITUACAO']);
        }
        $this->Conexao->fecharConexao();
